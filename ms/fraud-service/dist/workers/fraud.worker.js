...
exports.detectCyclesAndFlag = detectCyclesAndFlag;
// src/workers/fraud.worker.ts
const transaction_model_1 = __importDefault(require("../models/transaction.model"));
async function detectCyclesAndFlag(options) {
    const { windowHours = 24, maxLen = 4, minAmount = 0 } = options;
    const since = new Date(Date.now() - windowHours * 60 * 60 * 1000);
    // 1. fetch recent successful transactions
    const edgesRaw = await transaction_model_1.default.find({
        createdAt: { $gte: since },
        status: "SUCCESS",
    ...
    if (bulkOps.length) {
        const res = await transaction_model_1.default.collection.bulkWrite(bulkOps, { ordered: false });
